...
class lonlat_To_Odom(Node):
    ...
    def movingbase_callback(self, msg):
        if self.count == 0:
            ...
            self.count = 1
        self.theta_set = msg.orientation_covariance[0]
        self.get_logger().info(f"theta: {self.theta_set}")
    ...
    def conversion(self, coordinate, origin, theta):
       ...
       # %===X===%
        ...
        point = list(point) # リストに変換 
        point[1] = point[1] + 4.2 
        point = tuple(point) # 必要に応じて再びタプルに変換 
    ...
    def publish_lonlat_to_odom(self):
        ...
        if lonlat[0] is not None and lonlat[1] is not None and self.theta is not None:
            if lonlat[0] != 0 and lonlat[1] != 0:
                ...
                theta_rad = (-self.theta_set)/180*math.pi  -90/180*math.pi
                orientation_z, orientation_w = self.yaw_to_orientation(theta_rad)
                ...
                self.odom_msg.pose.pose.orientation.z = orientation_z
                self.odom_msg.pose.pose.orientation.w = orientation_w
